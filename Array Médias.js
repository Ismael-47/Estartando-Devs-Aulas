const alunos = [
  ...
  function media(nomeAluno) {
    let mediaAluno = nomeAluno.notas.reduce((soma, nota) => soma + nota) / 6 ;
    return {
      nome: nomeAluno.nome,
      notas: nomeAluno.notas,
      media: mediaAluno.toFixed(1)
    };
  }
  
  const alunosMedias = alunos.map(media);
 
  
  alunosMedias.forEach(aluno => {
    if (aluno.media > 6) {
      console.log(
